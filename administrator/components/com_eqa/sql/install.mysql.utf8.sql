...
CREATE TABLE `#__eqa_examseasons`(
	...
	`nexam` INT NOT NULL DEFAULT 0 COMMENT 'Số lượng môn thi',
	`nexamsession` INT NOT NULL DEFAULT 0 COMMENT 'Số lượng ca thi',
...
CREATE TABLE `#__eqa_reviews`(
    `exam_id` INT NOT NULL COMMENT 'Khóa ngoại: mã kỳ thi',
	`learner_id` INT COMMENT 'FK: thí sinh',
	`examiner1_id` INT COMMENT 'FK: CBChT1',
	`examiner2_id` INT COMMENT 'FK: CBChT2',
	`mark_before` REAL COMMENT 'Điểm TRƯỚC phúc khảo',
	`mark_after` REAL COMMENT 'Điểm SAU phúc khảo',
	`description` TEXT,
	`created_at` DATETIME,
	`created_by` VARCHAR(255),
	`updated_at` DATETIME,
	...
	`checked_out` INT DEFAULT NULL,
	`checked_out_time` DATETIME DEFAULT NULL,
	CONSTRAINT fk_eqa_reviews_exam FOREIGN KEY (`exam_id`)
		REFERENCES `#__eqa_exams`(`id`)
		ON DELETE RESTRICT,
	CONSTRAINT fk_eqa_reviews_examiner1 FOREIGN KEY (`examiner1_id`)
		REFERENCES `#__eqa_employees`(`id`)
		ON DELETE RESTRICT,
	CONSTRAINT fk_eqa_reviews_examiner2 FOREIGN KEY (`examiner2_id`)
		REFERENCES `#__eqa_employees`(`id`)
		ON DELETE RESTRICT,
	CONSTRAINT fk_eqa_reviews_learner FOREIGN KEY (`learner_id`)
		REFERENCES `#__eqa_learners`(`id`)
		ON DELETE RESTRICT
