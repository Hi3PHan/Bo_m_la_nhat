...
class ExamroomsController extends EqaAdminController {
	...
	public function import(): void
	{
		...
		for($sh=0; $sh<$sheetNumber; $sh++){
			 ...
			 * sau đây sẽ kiểm tra 10 thí sinh của phòng thi, xem số báo danh có tương ứng
			 * với mã HVSV hay không
			 */
			$size = min([10, sizeof($data)]);
			$testData = [];
			for($i=0; $i<$size; $i++)
			{
			...
			switch ($testType){
				case ExamHelper::TEST_TYPE_PAPER:
					$model->importPaperTest($examroomId, $examroomName, $data);
					break;
				default:
					$model->importNonpaperTest($examroomId, $examroomName, $data);
			} //Hết một sheet
