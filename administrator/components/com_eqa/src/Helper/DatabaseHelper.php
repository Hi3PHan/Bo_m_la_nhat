...
abstract class DatabaseHelper
{
	...
	static public function getExamInfo($examId): ExamInfo|null
	{
		...
		$db = self::getDatabaseDriver();
		$columns = $db->quoteName(
			array('a.id','a.name', 'a.testtype', 'a.usetestbank', 'a.duration', 'b.name',  'b.term',  'c.code', 'a.status'),
			array('id',  'name',   'testtype',   'usetestbank',   'duration', 'examseason', 'term', 'academicyear', 'status')
		);
		$query = $db->getQuery(true)
			->select($columns)
		...
		$query = $db->getQuery(true)
			...
			->where('a.exam_id='.(int)$examId.' AND b.debtor<>0');
		$db->setQuery($query);
    ...
    static public function getExamExaminees(int $examId, bool $allowedOnly = false)
    {
        $db = self::getDatabaseDriver();
	    $columns = $db->quoteName(
		    array('a.learner_id','a.code','b.code',       'b.lastname', 'b.firstname', 'a.attempt', 'c.pam1', 'c.pam2','c.pam','c.allowed', 'b.debtor','d.name',  'e.name',     'e.start',  'a.mark_final', 'a.module_mark', 'a.module_grade', 'a.conclusion'),
	...
	static public function getLearnerId(string $learnerCode)
	{
		$db     = self::getDatabaseDriver();
		$query  = $db->getQuery(true)
			->select('id')
	...
	static public function getLearnerIds(array $learnerCodes)
	{
		$db           = self::getDatabaseDriver();
		$learnerCodes = $db->quote($learnerCodes);
		$learnerCodeSet = '(' . implode(',', $learnerCodes) . ')';
		...
		$db->setQuery($query);
		$res = $db->loadColumn();
		if(sizeof($res) != sizeof($learnerCodes))
			return false;
		return $res;
	}
